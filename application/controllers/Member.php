<?php class Member extends CI_Controller {  
        ...
        if ($user) {  
            ...
            if ($user['is_active'] == 1) {  
                ...
                if (password_verify($password, $user['password'])) {  
                    ...
                    $this->session->set_flashdata('pesan', '<div class="alertalert-danger alert-message" role="alert">Password salah!!</div>');  
                    redirect('home');  
                }  
            }  
            else {  
    ...
    public function ubahProfil() {  
        ...
        $user = $this->ModelUser->cekData(['email' => $this->session->userdata('email')])->row_array();  
        foreach ($user as $a) {  
        $data = [ 'image' => $user['image'], 'user' => $user['nama'], 'em
        ail' => $user['email'], 'tanggal_input' => $user['tanggal_input'], ];  
        } 
        $this->form_validation->set_rules('nama', 'Nama Lengkap', 'required|t
        rim', [ 'required' => 'Nama tidak Boleh Kosong' ]);  
    ...
    public function updateTanggalAnggota()
    {
        $id_user = 1; // Ganti sesuai ID user yang ingin diubah
        $tanggal_baru = strtotime('2025-06-03');

        $this->db->set('tanggal_input', $tanggal_baru);
        $this->db->where('id', $id_user);
        $this->db->update('user');

        echo "Tanggal berhasil diupdate ke 3/6/2025.";
    }
