...
class Booking extends CI_Controller {  
    ...
    public function index() {  
        ...
        $data['booking'] = $this->ModelBooking->joinOrder($id)->result();$user = $this->ModelUser->cekData(['email' => $this->session->userdata('email')])->row_array();  
        foreach ($user as $a) {  
    ...
    public function hapusbooking() {  
      ...
      public function info() { 
          ...
          public function exportToPdf() {
              ...
              //Ambil output dari view bukti-pdf
              $this->load->view('booking/bukti-pdf', $data);
              $html = $this->output->get_output();
              //GUNAKAN $this->dompdf_gen, BUKAN $this->dompdf
              $this->dompdf_gen->set_paper('A4', 'landscape');
              $this->dompdf_gen->load_html($html);
              $this->dompdf_gen->render();
              ob_end_clean(); // Buang buffer
              $this->dompdf_gen->stream("bukti-booking-$id_user.pdf", array("Attachment" => 0));
            }
          } 
